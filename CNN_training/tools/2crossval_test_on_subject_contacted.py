...
if __name__ == '__main__':
    for rsn_i in range(iStart, iEnd + 1):
        ...
        for sub_i in sub_list:
            ...
            with open('/data1/wqy/Projects/S900_RSN/CNN_training/experiments/2corssval_test_on_sub_contacted.yaml') as f:
                content = yaml.load(f)
                ...
                content['TEST']['MODEL_DIR'] = 'output/2-fold_CrossVal_group1/%02d/' % (rsn_i)
                content['TEST']['RESULT_DIR'] = 'output/2-fold_CrossVal_group1/test_results/%02d/%d/' % (rsn_i, sub_i)
                pre_path = '/data1/wqy/Projects/S900_RSN/CNN_training/output/2-fold_CrossVal_group1/test_results/%02d/%d/' % (rsn_i, sub_i)
                if (not os.path.exists(pre_path)):
                    os.makedirs(pre_path)
                # print(content)

            with open('/data1/wqy/Projects/S900_RSN/CNN_training/experiments/2corssval_test_on_sub_contacted.yaml', 'w') as nf:
